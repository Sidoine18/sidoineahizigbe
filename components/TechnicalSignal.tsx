...
export default function TechnicalSignal({ steps, title, dense = false }: TechnicalSignalProps) {
  return (
    <div className="w-full">
      {title && <p className="signal-label mb-4 uppercase tracking-wider text-faint">{title}</p>}
      <div className="relative">
        <div className="absolute left-0 right-0 top-1/2 h-px bg-line -translate-y-1/2" aria-hidden="true" />
        <div
          className="absolute top-1/2 h-1.5 w-1.5 -translate-y-1/2 rounded-full bg-signal shadow-[0_0_8px_2px_rgba(37,99,168,0.45)] animate-pulse-travel"
          aria-hidden="true"
        />
